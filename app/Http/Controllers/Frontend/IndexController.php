...
class IndexController extends Controller
{
    public function Index()
    {
        ...
        $skip_category_0 = Category::skip(10)->first();
        $skip_product_0 = Product::where('status', 1)
                        ->where('category_id', $skip_category_0->id)->orderBy('id', 'DESC')->limit(5)->get();

        $skip_category_2 = Category::skip(9)->first();
        $skip_product_2 = Product::where('status', 1)
                        ->where('category_id', $skip_category_2->id)->orderBy('id', 'DESC')->limit(5)->get();

        $skip_category_8 = Category::skip(8)->first();
        $skip_product_8 = Product::where('status', 1)
                        ->where('category_id', $skip_category_8->id)->orderBy('id', 'DESC')->limit(5)->get();
        ...
        compact('skip_category_0', 'skip_product_0', 'skip_category_2', 'skip_product_2', 'skip_category_8', 'skip_product_8',
                'hot_deals','special_offer','new','special_deals'));
    } // End Method
    ...
    public function SearchProduct(Request $request)
    {
        ...
        $products = Product::where('product_name', 'LIKE', "%$item%")
                            ->select('product_name', 'product_slug', 'product_thambnail', 'selling_price', 'id')
                            ->limit(6)->get();
